/*
FlashMemory.cpp - A easy to use interface for the Spansion flash memory chips.
...
uint8_t SpiFlash::receive(uint8_t* buf, size_t n) {
...
void SpiFlash::send(const uint8_t* buf , size_t n) {
	// clear any data in RX FIFO
	SPI0_MCR = SPI_MCR_MSTR | SPI_MCR_CLR_RXF;
#if SPI_USE_8BIT_FRAME
	// initial number of bytes to push into TX FIFO
